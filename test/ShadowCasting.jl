...
@testset "shadow casting" begin
    ...
    @testset "cast_shadow" begin
        ...
        buildings.conv = is_convex(buildings.geometry)
        shadows_explicit = CompositeBuildings.cast_shadow_explicit.(to_points.(buildings.geometry), [0.5, -1, 0.3])
        shadows.conv = buildings.conv
        shadows_explicit.conv = buildings.conv
        project_local!(shadows)
        project_local!(shadows_explicit)
        filter!(:conv => identity, shadows)
        filter!(:conv => identity, shadows_explicit)

        @test nrow(shadows) == 885
        @test nrow(shadows_explicit) == 885
